...
public:
	struct Vertex
	{
		...
		Vertex(const XMFLOAT3& p, const XMFLOAT3& n, const XMFLOAT3& t, const XMFLOAT2& uv, const UINT & txNum )
			: Position(p), Normal(n), TangentU(t), TexC(uv), TextureNum(txNum){}
		Vertex(
			float px, float py, float pz, 
			float nx, float ny, float nz,
			float tx, float ty, float tz,
			float u, float v, UINT txNum)
			: Position(px,py,pz), Normal(nx,ny,nz),
			  TangentU(tx, ty, tz), TexC(u,v), TextureNum(txNum){}
	...
	///</summary>
	static float	getPalletHeight();
	void CreateBox(float width, float height, float depth, MeshData& meshData);

	void CreateLocation(float width, float height, float depth, MeshData& meshData);
	void ItemBox(float width, float height, float depth, MeshData& meshData);
