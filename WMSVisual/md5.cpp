...
//    sprintf(s+i*2, "%02x", digest[i]);
  ...
  sprintf_s(s + i*2, 1, "%02x", digest[i]);
...
inline DWORD MD5::rotate_left  (uint4 x, uint4 n){
  return (x << n) | (x >> (32-n))  ;
}
...
inline DWORD MD5::F            (uint4 x, uint4 y, uint4 z){
  return (x & y) | (~x & z);
}

inline DWORD MD5::G            (uint4 x, uint4 y, uint4 z){
  return (x & z) | (y & ~z);
}

inline DWORD MD5::H            (uint4 x, uint4 y, uint4 z){
  return x ^ y ^ z;
}

inline DWORD MD5::I            (uint4 x, uint4 y, uint4 z){
  return y ^ (x | ~z);
}
